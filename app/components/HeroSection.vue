...
<template>
  ...
  >
    ...
    <div class="cinema-editorial-grid absolute inset-0 pointer-events-none" aria-hidden="true" />
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-6 py-16 lg:py-20 min-h-[calc(100svh-4rem)] grid lg:grid-cols-[0.9fr_1.1fr] gap-10 lg:gap-6 items-center">
      <div class="flex flex-col items-start relative z-20">
        ...
        <!-- Title and subtitle use a CSS slide, not v-motion: they are visible from
             the first paint, so LCP does not wait for hydration. -->
        <h1 class="hero-rise text-[clamp(2.75rem,5.2vw,5.1rem)] font-extrabold leading-[1.28] tracking-[-0.06em] text-[#f6f7fb] max-w-[690px]">
          Metti ordine nel lavoro. <span class="cinema-text cinema-word">Lascia</span> <span class="cinema-text cinema-word">spazio</span> <span class="cinema-text cinema-word">alle persone.</span>
        </h1>

        <p class="hero-rise hero-rise-late text-base sm:text-lg text-[#a3a6b9] leading-relaxed max-w-xl mt-7">
...
.hero-rise{animation:heroRise .7s .08s cubic-bezier(.22,1,.36,1) both}.hero-rise-late{animation-duration:.58s;animation-delay:.18s}@keyframes heroRise{from{transform:translateY(24px)}}
